...
function isMod(sender) {
  return sender?.is_mod === true || sender?.badges?.some(b => b.type === 'moderator' || b.type === 'broadcaster') || false
}
...
bus.on('chat.message.sent', async (data) => {
  ...
  const userId = String(sender.id || sender.user_id || sender.username)
...
logger.log(TAG, 'Chat commands & reward handlers registered')
