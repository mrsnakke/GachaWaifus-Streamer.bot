...
const { resizeImage, uploadImageToGitHub } = require('../utils/imageUploader'); // Importar las utilidades de imagen
...
router.get('/endpoints', (req, res) => {
    ...
    const endpoints = {
        streamerbot: [
            `Tirada Simple (Gratis): http://${ipPlaceholder}:${port}/pull-single?userId=USER_ID&userName=USERNAME`,
            `Tirada Múltiple (x10 Gratis): http://${ipPlaceholder}:${port}/pull-multi?userId=USER_ID&userName=USERNAME`,
            `Tirada Simple (x1 Key): http://${ipPlaceholder}:${port}/pull-single-x1key?userId=USER_ID&userName=USERNAME`,
            `Tirada Múltiple (x10 Keys): http://${ipPlaceholder}:${port}/pull-multi?userId=USER_ID&userName=USERNAME&useKeys=true`,
            `Añadir Keys a Usuario: http://${ipPlaceholder}:${port}/add-keys?userId=USER_ID&userName=USERNAME&amount=CANTIDAD`,
            `Ver Info de Personaje: http://${ipPlaceholder}:${port}/view-character?userId=USER_ID&userName=USERNAME&characterName=NOMBRE_PERSONAJE`
        ],
        admin: [
            `Limpiar Todos los Datos: http://${ipPlaceholder}:${port}/admin/clear-all-data?confirm=true`,
            `Panel de Administración: http://${ipPlaceholder}:${port}/admin.html`
        ],
        trade: `Página de Tradeo: http://${ipPlaceholder}:${port}/trades.html`,
        keys: [
            `Obtener Keys de Usuarios (Admin): http://${ipPlaceholder}:${port}/admin/user-keys`,
            `Añadir Keys a Usuario (Admin): http://${ipPlaceholder}:${port}/admin/user-keys/add (POST con { username: "USERNAME", keys: CANTIDAD })`
        ]
    };
...
router.post('/character', upload.single('image'), async (req, res) => {
    ...
    if (imageFile) {
        try {
            const resizedImageBuffer = await resizeImage(imageFile.buffer);
            const filename = `${safeName}${path.extname(imageFile.originalname)}`;
            imageUrl = await uploadImageToGitHub(filename, resizedImageBuffer, rarity);
        } catch (uploadError) {
            console.error('Error al procesar y subir la imagen:', uploadError);
            return res.status(500).json({ error: 'Error al procesar y subir la imagen del personaje.' });
        }
    }
    ...
    try {
        ...
        if (newCharacter.stock !== undefined) {
            ...
            // Si es un personaje de temporada (5 o 6 estrellas y seasonal_banner), añadirlo también a seasonal_characters.json
            if ((rarity === '5_star' || rarity === '6_star') && banner === 'seasonal_banner') {
                const existingSeasonalChar = dataManager.seasonalCharactersConfig.characters.find(c => c.name === name);
                if (!existingSeasonalChar) {
                    dataManager.seasonalCharactersConfig.characters.push({ name, stock: newCharacter.stock });
...
router.put('/character/:oldName', upload.single('image'), async (req, res) => {
    ...
    try {
        ...
        // 1. Manejar el cambio de nombre del archivo JSON del personaje
        if (oldName !== name) {
            ...
            // Eliminar la entrada antigua del dataManager.characterData
            delete dataManager.characterData[oldName];
        }

        // 2. Manejar la actualización de la imagen
        let imageUrl = existingCharDetails.image_url;
        if (imageFile) {
            try {
                const resizedImageBuffer = await resizeImage(imageFile.buffer);
                const filename = `${safeNewName}${path.extname(imageFile.originalname)}`;
                imageUrl = await uploadImageToGitHub(filename, resizedImageBuffer, rarity);
            } catch (uploadError) {
                console.error('Error al procesar y subir la nueva imagen:', uploadError);
                return res.status(500).json({ error: 'Error al procesar y subir la nueva imagen del personaje.' });
            }
        }

        // 3. Crear el objeto de personaje actualizado
        const updatedCharacter = {
            ...existingCharDetails,
            name: name,
            rarity: rarity,
            image_url: imageUrl,
            description: existingCharDetails.description, // Mantener la descripción existente
            stock: (rarity === '5_star' || rarity === '6_star') && banner === 'seasonal_banner' ? parseInt(stock) || 0 : undefined
        };

        // 4. Escribir el archivo JSON del personaje actualizado
        await fs.writeFile(newCharFilePath, JSON.stringify(updatedCharacter, null, 2));

        // 5. Actualizar referencias en los banners (standard_banner.json y seasonal_banner.json)
        // Eliminar el personaje con el nombre antiguo de todos los banners
        for (const rarityKey in dataManager.standardBanner) {
            dataManager.standardBanner[rarityKey] = dataManager.standardBanner[rarityKey].filter(char => char !== oldName);
        }
        for (const rarityKey in dataManager.seasonalBanner) {
            dataManager.seasonalBanner[rarityKey] = dataManager.seasonalBanner[rarityKey].filter(char => char !== oldName);
        }

        // Añadir el personaje con el nuevo nombre al banner correcto
        const targetBanner = (banner === 'standard_banner') ? dataManager.standardBanner : dataManager.seasonalBanner;
        if (!targetBanner[rarity]) {
            targetBanner[rarity] = [];
        }
        if (!targetBanner[rarity].includes(name)) { // Evitar duplicados si ya existe por alguna razón
            targetBanner[rarity].push(name);
        }

        // Guardar los archivos de banner actualizados
        ...
        // 6. Actualizar dataManager.characterData
        dataManager.characterData[name] = updatedCharacter;
        // No es necesario reasignar dataManager.standardBanner/seasonalBanner ya que se modificaron directamente

        // 7. Actualizar gachaConfig.character_stocks
        if (oldName !== name && dataManager.gachaConfig.character_stocks && dataManager.gachaConfig.character_stocks[oldName] !== undefined) {
            dataManager.gachaConfig.character_stocks[name] = dataManager.gachaConfig.character_stocks[oldName];
            delete dataManager.gachaConfig.character_stocks[oldName];
        }
        ...
        // 8. Actualizar seasonal_characters.json si el personaje es de temporada
        const seasonalCharIndexOld = dataManager.seasonalCharactersConfig.characters.findIndex(c => c.name === oldName);
        if (seasonalCharIndexOld !== -1) {
            // Si el personaje era de temporada, actualizar su nombre y stock
            dataManager.seasonalCharactersConfig.characters[seasonalCharIndexOld].name = name;
            dataManager.seasonalCharactersConfig.characters[seasonalCharIndexOld].stock = updatedCharacter.stock;
        } else if ((rarity === '5_star' || rarity === '6_star') && banner === 'seasonal_banner') {
            // Si no era de temporada y ahora lo es, añadirlo
            dataManager.seasonalCharactersConfig.characters.push({ name, stock: updatedCharacter.stock });
        }
        // Si era de temporada y ya no lo es (cambio de banner/rarity), se debería eliminar.
        // La lógica de eliminación ya lo maneja, pero aquí nos aseguramos de que si cambia de ser seasonal a no-seasonal, se elimine de la lista.
        if (!((rarity === '5_star' || rarity === '6_star') && banner === 'seasonal_banner')) {
...
router.delete('/character/:name', async (req, res) => {
    ...
    try {
        ...
        // No es necesario eliminar la imagen local, ya que ahora se gestionan en GitHub.
        // Si se quisiera eliminar de GitHub, se necesitaría una función adicional en imageUploader.js
        // para interactuar con la API de GitHub para eliminar archivos.
        // Por ahora, solo se elimina la referencia en el JSON.
...
router.put('/gacha-config/character-stocks', async (req, res) => {
    ...
    try {
        ...
        for (const charName in updatedStocks) {
            ...
            // Sincronizar con seasonal_characters.json si es un personaje de temporada
            const seasonalCharIndex = dataManager.seasonalCharactersConfig.characters.findIndex(c => c.name === charName);
            if (seasonalCharIndex !== -1) {
                dataManager.seasonalCharactersConfig.characters[seasonalCharIndex].stock = updatedStocks[charName];
            }
        }
        await dataManager.saveSeasonalCharactersConfig(); // Guardar los cambios en seasonal_characters.json
...
router.post('/seasonal-characters-config/add-character', async (req, res) => {
    ...
    // Asegurarse de que el stock también se refleje en gacha_config.json
    if (!dataManager.gachaConfig.character_stocks) {
        dataManager.gachaConfig.character_stocks = {};
    }
    dataManager.gachaConfig.character_stocks[name] = initialStock;
    await dataManager.saveGachaConfig();
...
// --- ENDPOINTS PARA KEYS DE USUARIO ---

// Endpoint para obtener las keys de todos los usuarios
router.get('/user-keys', async (req, res) => {
    try {
        const userInventory = await dataManager.loadUserInventory();
        // Filtrar solo los usuarios que tienen la propiedad 'keys'
        const usersWithKeys = Object.keys(userInventory).reduce((acc, username) => {
            if (userInventory[username] && userInventory[username].keys !== undefined) {
                acc[username] = { keys: userInventory[username].keys };
            }
            return acc;
        }, {});
        res.json(usersWithKeys);
    } catch (error) {
        console.error('Error al obtener las keys de usuario:', error);
        res.status(500).json({ error: 'Error interno del servidor al cargar las keys de usuario.' });
    }
});

// Endpoint para añadir keys a un usuario
router.post('/user-keys/add', async (req, res) => {
    const { username, keys } = req.body;

    if (!username || keys === undefined || isNaN(keys) || keys <= 0) {
        return res.status(400).json({ error: 'Datos de entrada inválidos. Se requiere un nombre de usuario y una cantidad de keys positiva.' });
    }
    ...
    try {
        ...
        if (!userInventory[username]) {
            userInventory[username] = { keys: 0, inventory: [] }; // Inicializar si el usuario no existe
        }
        userInventory[username].keys += parseInt(keys);

        await dataManager.saveUserInventory(userInventory);
        res.status(200).json({ message: `Se han añadido ${keys} keys a ${username}. Total: ${userInventory[username].keys}` });
    } catch (error) {
        console.error(`Error al añadir keys al usuario ${username}:`, error);
        res.status(500).json({ error: 'Error interno del servidor al añadir keys.' });
    }
});
